VERSION 5.00
Begin VB.Form ImprimirAIntervalo 
   ...
   Begin VB.CommandButton CmdImpPDF 
      ...
      Height          =   1155
      Left            =   4410
      Picture         =   "ImprimirAIntervalo.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   6
   ...
   Begin VB.CommandButton CmdImpImpresora 
      ...
      Left            =   5670
      MaskColor       =   &H00E0E0E0&
      Picture         =   "ImprimirAIntervalo.frx":1FBA
      Style           =   1  'Graphical
      TabIndex        =   5
      Tag             =   "1"
   ...
   End
End
Attribute VB_Name = "ImprimirAIntervalo"
...
Dim Periodos_Consultado  As String
    TXTSQL = "SELECT * FROM COMPROBANTE WHERE NoComprobante>='" & Format(Txtdesde.Text, "00000000") & "' and NoComprobante<='" & Format(TxtHasta.Text, "00000000") & "'"
    'TXTSQL = "SELECT * FROM COMPROBANTE WHERE NoComprobante>=" & Txtdesde.Text & " and NoComprobante<=" & TxtHasta.Text
...
On Error GoTo ErrHandler
    ...
    Do Until REC1.EOF
        NoComprobante = Format(REC1.Fields("NoComprobante"), "00000000")
        'Abrir el reporte
        Screen.MousePointer = vbHourglass
...
On Error GoTo errores
   TXTSQL = "SELECT SUM(IvaRetenido) from comprobante where NoComprobante>='" & Format(Txtdesde.Text, "00000000") & "' and NoComprobante<='" & Format(TxtHasta.Text, "00000000") & "'"
    Set REC1 = DB.Execute(TXTSQL)
